...
class AssistantCommandRouter {
    ...
    fun route(
        ...
        resolveLauncherCommand(intent.command)?.let { launcherCommand ->
        ...
        if (isRepeatRequest(intent) && !context.lastResponse.isNullOrBlank()) {
            return AssistantAction.RepeatLastResponse
        }
    ...
    private fun resolveLauncherCommand(command: AssistantIntent.Command): AssistantAction.LauncherCommand? = when (command) {
    ...
    private fun isRepeatRequest(intent: AssistantIntent): Boolean {
        if (intent.command != AssistantIntent.Command.UNKNOWN) return false
        val input = intent.normalizedInput
        return input.contains("repeat") || input.contains("again") || input.contains("say that")
    }
}
